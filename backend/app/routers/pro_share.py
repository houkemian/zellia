...
from app.orm_loads import pro_share_with_target_user
from app.schemas.mappers import pro_share_to_item
from app.schemas.pro_share import ProShareAddRequest, ProShareMyResponse
...
@router.get("/shares/my", response_model=ProShareMyResponse)
def list_my_pro_shares(
    ...
    rows = db.execute(
        ...
        .options(pro_share_with_target_user())
        .order_by(ProShare.id.asc())
    ).scalars().all()
    # 使用扁平模型防止循环引用：仅映射为 ProShareItemResponse，不返回 ORM
    shared_users = [pro_share_to_item(row) for row in rows]
    return ProShareMyResponse(
