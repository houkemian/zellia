...
1. 亲情家庭包 (Family Shared Allocation) — *规划中曾与实现对齐前的草案*
    ...
    后端实现（草案，未按此实现）：
        ...
        FamilyGroup 模型关联 owner_id。

        鉴权中间件：在调用敏感接口（如开启预警）时，检查 current_user 所属家庭组的 owner 是否拥有活跃订阅。

2. 专业就医支持 (Professional Support)

    无限历史存档：

        FREE 用户：后端接口返回最近 30 天体征流水；超过 30 天的数据在数据库中保留但不返回前端（作为“唤醒付费”的诱饵）。

        PRO 用户：解除 offset/limit 限制，支持查看数年的完整健康曲线。

    高保真 PDF 报表：

        功能：支持自定义时间段（如：近三月、手术后、化疗期）。

        内容：包含“体征散点图”、“平均波动分析”、“漏服率统计”以及“紧急事件记录”。

        技术项：前端采用 printing + pdf 库，后端提供经过脱敏处理的聚合数据 JSON。

3. 安全守护中心 (Safety Guard Center)

    实时异常推送：

        逻辑：后端 vitals 路由拦截。当收缩压 > 140 或血糖 < 3.9 时，查询所有关联的 Caregiver。

        PRO 特权：仅当 Caregiver 为 PRO（或所在组为 PRO）时，触发 Firebase Cloud Messaging (FCM) 发送高优先级（High Priority）推送。

    自动化周报 (Weekly Digest)：

        技术：基于 APScheduler 的定时任务，每周一早 8:00 聚合数据，通过 Jinja2 模板渲染 HTML 邮件发送。

📱 桌面/锁屏实时挂件矩阵 (抬眼即见的安心)

    核心痛点：异地看护的子女每天都在隐隐担心父母的健康，但频繁打开 App 查看既繁琐又像在“查岗”。

    PRO 权益：解锁精美的 iOS 锁屏挂件 / Android 桌面小组件 (Widgets)。

    功能表现：无需打开 App，点亮手机屏幕即可直接看到父母今日“是否已按时服药”、“最新血压水平”以及“最后更新时间”。

    吸引力本质：它将低频的工具转化为高频的“安心安慰剂”。子女每天看手机几十次，每一次都能看到代表父母安好的绿色指标，这种即时获得的情绪价值是极佳的续费抓手。

    落地成本：极低。纯客户端本地开发（利用 Flutter 的 home_widget 库同步后端轻量状态），完全不消耗服务器额外计算资源。

❤️ 亲情互动流与“全屏关怀反馈” (打破单向监控冰冷感)

    核心痛点：长期单向记录血压和吃药数据极其枯燥，长辈容易产生“做任务”的厌烦感甚至放弃打卡。

    PRO 权益：解锁双向亲情激励系统。

    功能表现：

        远程撒花：子女在端上看到父母今日指标正常后，可一键发送“点赞/鲜花/拥抱”。长辈打开 App 时，屏幕会触发全屏精美动效（如满屏飘落的爱心）和自定义大字留言：“妈妈今天真棒，继续保持！”

        一键戳一戳：当系统检测到长辈超时未服药时，子女端可一键发送带有专属提示音的“提醒问候”。

    吸引力本质：巧妙利用轻量级社交激励，把冰冷的医疗监控转化为温暖的孝心传达纽带，极大提升长辈端的使用黏性与留存率。

    落地成本：纯前端动效集成（引入开源的 Lottie 或 Rive 动画组件），基于现有的 FCM 推送通道触发即可。